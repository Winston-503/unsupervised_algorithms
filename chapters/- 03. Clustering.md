...
According to this algorithm, clusters are high-density regions (where the data points are located close to each other) separeted by low-density regions (where the data points are located far to each other). The ability to calculate the density appears due to the presence of the concept of *core sample* - central concept of the DBSCAN - which means a sample located in the area of high density.

Therefore, a cluster is a group of core samples located close to each other and some non-core samples located close to core samples. Other samples are defined as outliers (or anomalies) and do not belong to any cluster.
...
**Cons**:
- Need to select and tune the density parameter;
...
**Pros**:
+ Knowledge about the number of clusters is not necessary;
+ As a result, we also have standard representatives of a cluster. Unlike K-Means, these instances are not just avegare values, but real objects from the train dataset.
